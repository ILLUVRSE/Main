...
type SignResult = { kid: string; alg: string; signature: string };
type CollectedSignature = { signerId: string; kid: string; alg: string; signature: string };

export interface SignerClient {
  id: string;
  sign(digestBuf: Buffer): Promise<SignResult>;
  verify(signatureBase64: string, digestBuf: Buffer): Promise<boolean>;
}
...
/** Mock signer wrapper */
class MockSignerClient implements SignerClient {
  id: string;
  constructor(id = 'mock') {
    this.id = id;
  }
  async sign(digestBuf: Buffer) {
    return mockSigner.signAuditHash(digestBuf);
  }
  async verify(signatureBase64: string, digestBuf: Buffer) {
    return mockSigner.verifySignature(signatureBase64, digestBuf);
  }
}

/** Proxy signer wrapper (uses memory-layer/service/audit/signingProxyClient.ts) */
class ProxySignerClient implements SignerClient {
  id: string;
  ...
  async verify(signatureBase64: string, digestBuf: Buffer) {
    return signingProxy.verifySignature(signatureBase64, digestBuf);
  }
}

/** KMS signer wrapper for a particular KMS KeyId */
class KmsSignerClient implements SignerClient {
  id: string;
  keyId: string;
  client: KMSClient;
  alg: string;

  constructor(keyId: string, alg = 'rsa-sha256') {
    this.keyId = keyId;
    this.id = `kms:${keyId}`;
    this.client = new KMSClient({ region: process.env.AWS_REGION || process.env.AWS_DEFAULT_REGION || 'us-east-1' });
    this.alg = alg.toLowerCase();
  }

  async sign(digestBuf: Buffer): Promise<SignResult> {
    if (!Buffer.isBuffer(digestBuf)) throw new Error('digestBuf must be Buffer');
    ...
    if (this.alg === 'hmac-sha256' || this.alg === 'hmac') {
      ...
      if (!resp?.Mac) throw new Error('KMS GenerateMac returned no Mac');
      return { kid: this.keyId, alg: 'hmac-sha256', signature: Buffer.from(resp.Mac).toString('base64') };
    }
    ...
    if (this.alg === 'rsa-sha256' || this.alg === 'rsa') {
      ...
      return { kid: this.keyId, alg: 'rsa-sha256', signature: Buffer.from(resp.Signature).toString('base64') };
    }
    ...
    if (this.alg === 'ed25519' || this.alg === 'ed25519-sha') {
      ...
      const resp = await this.client.send(cmd);
      if (!resp?.Signature) throw new Error('KMS Sign returned no Signature');
      return { kid: this.keyId, alg: 'ed25519', signature: Buffer.from(resp.Signature).toString('base64') };
    }

    throw new Error(`Unsupported KMS signer alg: ${this.alg}`);
  }

  async verify(signatureBase64: string, digestBuf: Buffer): Promise<boolean> {
    if (!Buffer.isBuffer(digestBuf)) throw new Error('digestBuf must be Buffer');
...
export class MultiSigCoordinator {
  ...
  constructor(signers: SignerClient[], threshold = 3) {
   ...
   */
  async collectSignatures(digestBuf: Buffer): Promise<CollectedSignature[]> {
    const promises = this.signers.map(async (s) => {
      try {
        const res = await s.sign(digestBuf);
        return { signerId: s.id, kid: res.kid, alg: res.alg, signature: res.signature } as CollectedSignature;
      } catch (err) {
        // log and continue
        console.error(`[multi] signer ${s.id} failed:`, (err as Error).message || String(err));
        return null as unknown as null;
      }
    });

    const results = await Promise.all(promises);
    return results.filter(Boolean) as CollectedSignature[];
  }
  ...
  async verifyCombinedProof(proof: {
  ...
  }): Promise<{ ok: boolean; validCount: number; errors: string[] }> {
    const digestBuf = Buffer.from(proof.digestHex, 'hex');
    let validCount = 0;
    const errors: string[] = [];
    ...
    for (const sig of proof.signatures ?? []) {
      ...
      try {
        ...
        if (ok) validCount += 1;
        else errors.push(`signature invalid for signer ${sig.signerId}`);
      } catch (err) {
        errors.push(`verify error for signer ${sig.signerId}: ${(err as Error).message || String(err)}`);
      }
    }
   ...
   */
  async createCombinedProof(digestBuf: Buffer): Promise<{
    digestHex: string;
    threshold: number;
    requestedAt: string;
    signatures: CollectedSignature[];
    ...
    for (const sig of signatures) {
      try {
        const signer = this.signers.find((s) => s.id === sig.signerId);
        if (!signer) continue;
        if (await signer.verify(sig.signature, digestBuf)) okCount += 1;
      } catch {
        // ignore
      }
    }
...
export function createSignerFromSpec(spec: string): SignerClient {
  // spec format: "mock", "proxy", "proxy:<id>", "kms:<keyId>[:alg]" or "kms:<keyId>"
  if (!spec || typeof spec !== 'string') throw new Error('invalid signer spec');
...
function parseArgCli(name: string): string | undefined {
  const prefix = `--${name}=`;
...
if (require.main === module) {
  mainCli().catch((err) => {
    console.error('multisig failed:', (err as Error).message || String(err));
    process.exit(10);
  });
}
