...
function truncateTextForEmbedding(text, maxChars = 3000) {
...
async function readFileSample(absPath, maxChars = 3000) {
    try {
...
function isSensitiveRelPath(rel) {
    ...
    if (/\.(env|pem|key|p12|jks|crt|csr)$/.test(lower))
        return true;
    // dot env files
    ...
    if (lower.startsWith("secrets/") || lower.includes("/secrets/"))
        return true;
    if (lower === "config/production.yml" || lower.endsWith("/config/production.yml"))
        return true;
    // common config files that might contain secrets
    if (lower.includes("config/") && (lower.endsWith("credentials.json") || lower.endsWith("credentials.yml")))
        return true;
    // node_modules, build artifacts, binary assets are not useful (already skipped by walk ignore)
    if (lower.startsWith("node_modules/") || lower.startsWith("dist/") || lower.startsWith("build/"))
        return true;
    return false;
}
/** Request embeddings from OpenAI-style endpoint. Returns vector or null on failure. */
async function requestEmbedding(text, model = DEFAULT_MODEL) {
    try {
...
export async function buildIndex(options = {}) {
    ...
    async function walk(dir) {
        ...
        for (const ent of ents) {
            ...
            if (ent.isDirectory()) {
                ...
                if (/\.(png|jpg|jpeg|gif|wasm)$/.test(name))
                    continue;
    ...
    const idx = { updatedAt: new Date().toISOString(), model, items: [] };
    let count = 0;
...
export async function upsertFile(relPath, options = {}) {
    const model = options.model || DEFAULT_MODEL;
    const maxChars = options.maxChars ?? 3000;
    const abs = path.resolve(REPO_PATH, relPath);
    const relNormalized = path.relative(REPO_PATH, abs).replace(/\\/g, "/");
    if (isSensitiveRelPath(relNormalized))
        throw new Error("Refuse to upsert sensitive file");
    const sample = await readFileSample(abs, maxChars);
    if (!sample)
 ...
 */
export async function queryEmbeddings(queryText, opts = {}) {
    ...
    if (queryVec && hasVectors) {
        ...
        for (const it of idx.items) {
            ...
            if (score > 0)
                hits.push({ path: it.path, score, sample: it.textSample });
        }
    }
