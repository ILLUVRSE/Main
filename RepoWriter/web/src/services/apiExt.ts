...
export async function validatePatchesExt(patches: PatchObj[], opts?: { testCommand?: string[]; timeoutMs?: number }) {
  const body: any = { patches };
  if (opts?.testCommand) body.testCommand = opts.testCommand;
  if (typeof opts?.timeoutMs === "number") body.timeoutMs = opts.timeoutMs;
 ...
 */
export async function rollbackCommitOrMetadata(args: { commitSha?: string; rollbackMetadata?: any }) {
  ...
  try {
    ...
    if (!res.ok) throw new Error(j?.error || String(j) || `HTTP ${res.status}`);
    return j;
  } catch (err) {
