...
async function fetchS3Object(s3url: string): Promise<Buffer> {
  if (!s3url.startsWith('s3://')) throw new Error('from-s3 must be s3://bucket/key');
  ...
  const resp: GetObjectCommandOutput = await client.send(cmd);
...
function computeSha256Hex(buf: Buffer | string): string {
  return crypto.createHash('sha256').update(Buffer.isBuffer(buf) ? buf : Buffer.from(String(buf), 'utf8')).digest('hex');
}

async function writeTempFile(prefix = 'audit-export-', ext = '.json', content: Buffer | string): Promise<string> {
...
function runCommandSync(cmd: string, args: string[], env?: NodeJS.ProcessEnv) {
...
async function main() {
  ...
  if (fromS3) {
    ...
    const p = path.resolve(String(fromFile));
  ...
  if (compareManifest) {
    const manifestPath = path.resolve(String(compareManifest));
    ...
    try {
      ...
      console.error('invalid manifest JSON:', (err as Error).message || String(err));
      process.exit(4);
    }
...
if (require.main === module) {
  main().catch((err) => {
    console.error('verificationCliWrapper failed:', (err as Error).message || String(err));
    process.exit(10);
  });
}
