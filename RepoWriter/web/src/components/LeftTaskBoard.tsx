...
export type Task = {
  ...
  updatedAt?: string;
  status: TaskStatus;
  plan?: any; // Plan object returned by planner; keep `any` so we can evolve shape
...
export default function LeftTaskBoard() {
  const [tasks, setTasks] = useState<Task[]>(() => {
    ...
    return [
      ...
      },
      {
        ...
        prompt:
          "Add a TypeScript utility `utils/summarize.ts` with a `summarize(text: string): string` function that returns the first sentence.",
        createdAt: now,
        status: "draft",
        plan: undefined,
        lastError: null
  ...
  async function runTaskPlan(taskId: string) {
  ...
  function setTaskField(id: string, fields: Partial<Task>) {
    const t = tasks.find((x) => x.id === id);
    if (!t) return;
    upsertTask({ ...t, ...fields, updatedAt: new Date().toISOString() });
  }

  function applyTaskPlan(id: string) {
  ...
  return (
    <div style={{ padding: 16, height: "100%", boxSizing: "border-box", display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <h3 style={{ margin: 0, color: "var(--text)" }}>Task Board</h3>
      ...
      {/* New task editor */}
      <div style={{ border: "1px solid rgba(255,255,255,0.04)", padding: 10, borderRadius: 8, background: "var(--surface)" }}>
        <input
          placeholder="Task title"
          value={newTitle}
          onChange={(e) => setNewTitle(e.target.value)}
          style={{ width: "100%", padding: 8, borderRadius: 6, border: "1px solid rgba(0,0,0,0.06)", marginBottom: 8 }}
        />
        <textarea
          placeholder="Describe the task in narrative language (this becomes the prompt for the model)"
          value={newPrompt}
          onChange={(e) => setNewPrompt(e.target.value)}
          style={{ width: "100%", padding: 8, borderRadius: 6, border: "1px solid rgba(0,0,0,0.06)", minHeight: 84 }}
        />
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 8 }}>
          <button
            ...
            style={{ padding: "6px 10px", borderRadius: 8, background: "transparent", border: "none", color: "var(--muted)" }}
          >
            Clear
          </button>
          <button
        ...
        {tasks.map((t) => (
          ...
          >
            <TaskCard
              ...
              onRun={() => runTaskPlan(t.id)}
              onImport={() => applyTaskPlan(t.id)}
              onSave={(fields) => setTaskField(t.id, fields)}
            />
          </div>
        ))}
      </div>
