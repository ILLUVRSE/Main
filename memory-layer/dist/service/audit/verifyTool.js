...
const auditChain_1 = __importDefault(require("./auditChain"));
...
async function main() {
    ...
    try {
        ...
        let lastHashSeen = null;
        let anyFailures = false;
        let idx = 0;
        console.log(`Verifying ${res.rows.length} audit events...`);
        for (const row of res.rows) {
            idx += 1;
            const canonical = auditChain_1.default.canonicalizePayload(row.payload ?? null);
            const computedDigestHex = auditChain_1.default.computeAuditDigest(canonical, row.prev_hash ?? null);
            ...
            if (signaturePresent) {
                try {
                    ...
                    sigValid = await auditChain_1.default.verifySignature(row.signature, digestBuf);
                }
                catch (err) {
                    sigError = err.message || String(err);
                }
            }
            const rowIssues = [];
            if (!chainOk)
                rowIssues.push('CHAIN_BROKEN(prev_hash != previous.hash)');
            if (!hashMatches)
                rowIssues.push('HASH_MISMATCH(computed != stored)');
            if (!signaturePresent)
                rowIssues.push('UNSIGNED');
            else if (!sigValid) {
                rowIssues.push(sigError ? `SIG_INVALID(${sigError})` : 'SIG_INVALID');
            }
            if (rowIssues.length)
                anyFailures = true;
            console.log(`[${idx.toString().padStart(3)}] id=${row.id} ts=${row.created_at} type=${row.event_type} manifestSig=${row.manifest_signature_id ?? 'n/a'}`);
            ...
            if (sigError)
                console.log(`       sig_error=${sigError}`);
            if (rowIssues.length)
        ...
        console.error('ERROR running verification:', err.message || String(err));
        process.exitCode = 4;
    }
    finally {
        await client.end();
