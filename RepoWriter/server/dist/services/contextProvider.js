...
async function readFileSafeLimited(absPath, maxBytes) {
    try {
...
function extractSnippet(content, maxLines = 6) {
...
function heuristicSummary(content, maxChars = 400) {
...
function truncateContentToTokens(content, maxTokens) {
...
function lexicalScore(promptWords, filename, contentSample) {
    ...
    async function walk(dir) {
        ...
        for (const ent of ents) {
            ...
            if (ent.isDirectory()) {
                ...
                if (name.endsWith(".png") || name.endsWith(".jpg") || name.endsWith(".jpeg") || name.endsWith(".gif") || name.endsWith(".wasm"))
                    continue;
 ...
 */
export async function buildContext(prompt, opts = {}) {
    ...
    for (const rel of candidates) {
        try {
            ...
            if (score > 0)
                scored.push({ path: rel, score, sample: content });
        }
        catch {
    ...
    for (const c of scored) {
        ...
        if (totalTokens >= conf.tokenBudget)
            break;
        if (c.score < conf.minScore)
            break;
        try {
