...
export const SANDBOX_ALLOWED_ROLES = (process.env.SANDBOX_ALLOWED_ROLES || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
// Local LLM
...
if (SANDBOX_ENABLED && !SANDBOX_IMAGE && !SANDBOX_COMMAND) {
    try {
        console.warn("[config] SANDBOX_ENABLED=1 but SANDBOX_IMAGE and SANDBOX_COMMAND are not set. " +
