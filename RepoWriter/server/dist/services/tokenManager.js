...
export function estimateTokensWithMargin(text, marginPercent = 0.1) {
 ...
 */
export function splitTextByTokens(text, maxTokens, overlapTokens = 0) {
    if (!text)
        return [];
    if (maxTokens <= 0)
 ...
 */
export function ensureItemsWithinTokenBudget(items, tokenBudget) {
    const out = [];
    let total = 0;
