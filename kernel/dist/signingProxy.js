...
const kms_1 = __importDefault(require("./config/kms"));
const signingProvider_1 = require("./signingProvider");
const kmsConfig = (0, kms_1.default)();
const kmsProvider = kmsConfig.endpoint
    ? (0, signingProvider_1.createSigningProvider)(kmsConfig, 'kms')
    : null;
const localProvider = new signingProvider_1.LocalSigningProvider(kmsConfig.signerId);
async function signManifest(manifest) {
    const request = (0, signingProvider_1.prepareManifestSigningRequest)(manifest);
    if (!kmsProvider) {
...
async function signData(data) {
    const request = (0, signingProvider_1.prepareDataSigningRequest)(data);
    if (!kmsProvider) {
        if (kmsConfig.requireKms) {
            throw new Error('REQUIRE_KMS=true but KMS_ENDPOINT is not configured');
        }
