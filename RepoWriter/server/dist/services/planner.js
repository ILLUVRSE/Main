 ...
 */
function buildUserPayload(prompt, memory = [], contextFragment) {
 ...
 */
function normalizePlan(raw, contextMeta) {
...
export async function planEdits(prompt, memory = []) {
    ...
    try {
        ...
        if (ctx && Array.isArray(ctx.files) && ctx.files.length > 0) {
            ...
            contextMeta = { context: { files: ctx.files.map((f) => ({ path: f.path, tokensEstimate: f.tokensEstimate })), totalTokens: ctx.totalTokens } };
        }
    }
    catch (err) {
