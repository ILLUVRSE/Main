...
exports.setSentinelClient = setSentinelClient;
exports.resetSentinelClient = resetSentinelClient;
exports.getSentinelClient = getSentinelClient;
exports.recordEvent = recordEvent;
exports.enforcePolicyOrThrow = enforcePolicyOrThrow;
...
function setSentinelClient(c) {
    client = c || defaultClient;
}
function resetSentinelClient() {
...
async function recordEvent(type, payload) {
    try {
        ...
        // eslint-disable-next-line no-console
        console.warn('[sentinel] recordEvent failed:', e.message || e);
    }
}
async function resolveDecision(maybeDecision) {
    if (maybeDecision && typeof maybeDecision.then === 'function') {
        return (await maybeDecision);
    }
...
function summarizePrincipal(ctx, decision) {
...
function summarizeContext(ctx) {
    ...
    if (allocation && typeof allocation === 'object') {
        summary.allocation = {
            entityId: ensureString(allocation.entityId) ||
    ...
    const requestId = ensureString(ctx.requestId) ||
...
async function auditPolicyDecision(policyName, ctx, decision) {
    ...
    try {
        await (0, auditStore_1.appendAuditEvent)('policy.decision', payload);
    }
    catch (err) {
        // eslint-disable-next-line no-console
        console.warn('[sentinel] failed to append policy.decision audit event:', err.message || err);
    }
}
async function enforcePolicyOrThrow(policyName, ctx) {
    try {
        const c = client;
        let decision;
        ...
        if (decision.allowed === false) {
            const err = new Error('policy.denied');
        ...
        const wrapped = new Error('sentinel.evaluate_error');
