...
r.post("/stream", async (req, res, next) => {
  try {
    const { prompt, memory = [], contextOptions = {}, conversationId } = req.body as { prompt: string; memory?: string[]; contextOptions?: any; conversationId?: string };
...
r.post("/plan", async (req, res, next) => {
  try {
    ...
    try {
      if (conversationId) convManager.addUserMessage(conversationId, prompt);
    } catch {
      // ignore
    }
