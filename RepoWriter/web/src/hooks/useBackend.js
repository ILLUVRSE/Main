...
function readSettings() {
    const backend = localStorage.getItem(LS_KEYS.backend) || "openai";
...
export default function useBackend() {
    ...
    useEffect(() => {
        ...
        window.addEventListener("repowriter:settingsChanged", handler);
        return () => window.removeEventListener("repowriter:settingsChanged", handler);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);
    const setBackend = useCallback((backend) => {
        setSettings((s) => {
            const next = { ...s, backend };
    ...
    const setLocalUrl = useCallback((url) => {
        setSettings((s) => {
    ...
    const setLocalModel = useCallback((m) => {
        setSettings((s) => {
