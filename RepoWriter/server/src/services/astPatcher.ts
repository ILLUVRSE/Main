...
type ApplyOpts = {
  // prefer AST approach; fallback to regex if AST unavailable
  preferAst?: boolean;
};

/** Helper: naive bracket matcher to find the end index of a block starting at openPos. */
function findMatchingBraceIndex(text: string, openPos: number) {
  let i = openPos;
    ...
    if (inTemplate) {
      ...
      if (ch === "$" && text[i + 1] === "{") {
        ...
        while (i < len && innerDepth > 0) {
          if (text[i] === "}" && text[i - 1] !== "\\") innerDepth--;
          else if (text[i] === "{" && text[i - 1] !== "\\") innerDepth++;
          i++;
        }
        continue;
      }
...
function replaceDeclarationNaive(content: string, name: string, newCode: string) {
...
async function replaceWithRecast(content: string, name: string, newCode: string) {
  try {
    ...
    try {
      ...
      parser = {
        parse(source: string) {
          return (babelParser as any).parse(source, {
            sourceType: "module",
            plugins: [
              "typescript",
              "jsx",
    ...
    recast.types.visit(ast, {
      visitFunctionDeclaration(pathNode: any) {
      ...
      visitClassDeclaration(pathNode: any) {
        const node = pathNode.node;
        if (node.id && node.id.name === name) {
          const newAst = recast.parse(newCode, { parser });
          const newNode = newAst.program.body[0];
      ...
      visitVariableDeclaration(pathNode: any) {
 ...
 */
export async function applyReplacements(replacements: FileReplacement[], opts: ApplyOpts = {}): Promise<AppliedEntry[]> {
  const results: AppliedEntry[] = [];
  for (const r of replacements) {
    const rel = r.path;
    if (!rel) throw new Error("Missing path in replacement");
