 ...
 */
export async function ensureGitUser(): Promise<void> {
  try {
 ...
 */
export async function commitFiles(files: string[], message: string): Promise<string> {
  await ensureGitUser();
 ...
 */
export async function pushToRemote(force = false): Promise<{ ok: boolean; error?: string }> {
