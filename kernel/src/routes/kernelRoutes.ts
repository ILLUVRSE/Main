...
/** Helper: decide if principal is required (production) */
function requirePrincipalOrFail(principal: any, res: Response) {
  if (ENV === 'production' && !principal) {
...
export default function createKernelRouter(): Router {
   ...
   */
  router.get('/kernel/division/:id', async (req: Request, res: Response, next: NextFunction) => {
  ...
  router.post('/kernel/agent', idempotencyMiddleware, async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (ENV === 'production') {
      if (!principal) return res.status(401).json({ error: 'unauthenticated' });
      if (!hasAnyRole(principal, Roles.SUPERADMIN) && !hasAnyRole(principal, Roles.OPERATOR)) {
        return res.status(403).json({ error: 'forbidden' });
      }
    }
   ...
   */
  router.get('/kernel/agent/:id/state', async (req: Request, res: Response, next: NextFunction) => {
    const id = req.params.id;
    const principal = (req as any).principal || getPrincipalFromRequest(req);
    if (ENV === 'production' && !principal) return res.status(401).json({ error: 'unauthenticated' });
  ...
  router.post('/kernel/allocate', idempotencyMiddleware, async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (ENV === 'production') {
      if (!principal) return res.status(401).json({ error: 'unauthenticated' });
      if (!hasAnyRole(principal, Roles.SUPERADMIN) && !hasAnyRole(principal, Roles.DIVISION_LEAD) && !hasAnyRole(principal, Roles.OPERATOR)) {
        return res.status(403).json({ error: 'forbidden' });
      }
    }
   ...
   */
  router.get('/kernel/audit/:id', async (req: Request, res: Response, next: NextFunction) => {
    const id = req.params.id;
    const principal = (req as any).principal || getPrincipalFromRequest(req);
    if (ENV === 'production') {
      if (!principal) return res.status(401).json({ error: 'unauthenticated' });
      if (!hasAnyRole(principal, Roles.SUPERADMIN) && !hasAnyRole(principal, Roles.AUDITOR)) {
        return res.status(403).json({ error: 'forbidden' });
      }
    }
   ...
   */
  router.get('/kernel/reason/:node', async (req: Request, res: Response) => {
