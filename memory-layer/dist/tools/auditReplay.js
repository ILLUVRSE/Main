...
const auditChain_1 = require("../service/audit/auditChain");
const kmsAdapter = __importStar(require("../service/audit/kmsAdapter"));
...
async function fetchFromS3(s3url) {
    if (!s3url.startsWith('s3://'))
        throw new Error('from-s3 must be an s3:// URL');
    ...
    if (!resp.Body)
        throw new Error('empty object from s3');
...
async function readInput() {
    ...
    let raw;
    if (fromFile) {
        raw = fs_1.default.readFileSync(fromFile, 'utf8');
    }
    else {
        raw = await fetchFromS3(fromS3);
    }
...
async function main() {
    ...
    try {
        ...
        for (const ev of toProcess) {
            ...
            const canonical = (0, auditChain_1.canonicalizePayload)(ev.payload ?? null);
            const computedDigest = (0, auditChain_1.computeAuditDigest)(canonical, prevHash);
            ...
            if (ev.signature) {
                try {
                    ...
                    if (!sigOk) {
                        ...
                        if (!force)
                            throw new Error('Signature invalid - aborting (use --force to override)');
                        else
                            console.warn('  --force specified: continuing despite invalid signature');
                    }
                    else {
                    ...
                    console.error('  SIG_CHECK_ERROR:', err.message || String(err));
                    if (!force)
                        throw err;
                    else
                        console.warn('  --force specified: continuing despite signature verification error');
                }
            }
            else {
            ...
            const headRes = await client.query('SELECT hash FROM audit_events ORDER BY created_at DESC LIMIT 1 FOR UPDATE');
      ...
      `;
        ...
        console.error('Replay failed:', err.message || String(err));
        process.exitCode = 3;
    }
    finally {
        await client.end();
