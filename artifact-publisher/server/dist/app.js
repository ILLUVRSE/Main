...
export const createApplication = (overrides = {}) => {
    ...
    const checkoutService = new CheckoutService(new StripeMock(config.stripePublicKey), new LedgerMock(config.financeLedgerId), proofService, new LicenseService(), new DeliveryService(config.deliveryKey), kernelClient, new OrderRepository(), config.deterministicSalt);
