...
async function callOpenAICompatibleOnce(system, user, model = "gpt-4o-mini") {
    if (!LOCAL_LLM_URL)
...
async function callTextGenerationWebuiOnce(prompt) {
    if (!LOCAL_LLM_URL)
        throw new Error("LOCAL_LLM_URL not configured");
...
export async function generateLocalPlan(prompt) {
    ...
    try {
        ...
        const content = (choice && (choice.message?.content ?? choice.text)) ?? (typeof r === "string" ? r : null);
        ...
        try {
            ...
            const content = (choice && (choice.message?.content ?? choice.text)) ?? (typeof r2 === "string" ? r2 : null);
            if (typeof content === "string") {
                const maybe = safeJsonParse(content);
                if (maybe)
                    return maybe;
 ...
 */
export async function* streamLocalPlan(system, user) {
    if (!LOCAL_LLM_URL)
        throw new Error("LOCAL_LLM_URL not configured");
    ...
    try {
        ...
        try {
            ...
            const bodyAny2 = res2.body;
            if (bodyAny2 && typeof bodyAny2.getReader === "function") {
                const reader2 = bodyAny2.getReader();
            ...
            for await (const c of res2.body) {
            ...
            throw new Error(`Local LLM streaming failed: ${String(err?.message ?? err)} ; fallback failed: ${String(err2?.message ?? err2)}`);
        }
    }
}
